...
6. #[
    ...
    + What do you observe about the geometric relationship between the electric field vector and the equipotential
      lines?
    ...
    + Calculate the magnitude of the electric field vectors at at least three points on your grid.
      ...
      - Since $V = - integral arrow(E) dot dd(arrow(r))$, we can deduce that
        $abs(V) = abs(- integral E dd(r)) = abs(- integral (k q)/r^2 dd(r)) = (k q)/r$.
      ...
      + At $r = #qty(100, "cm") = #qty("1.00", "m")$, $V = #qty(9.017, "V")$ so
        $E = V / r = #qty(9.017, "V") / #qty("1.00", "m") = #qty(9.017, "V/m")$.

      + At $r = #qty(158.5, "cm") = #qty("1.585", "m")$, $V = #qty(5.674, "V")$ so
        $E = V / r = #qty(5.674, "V") / #qty("1.585", "m") = #qty(3.579, "V/m")$.

      + At $r = #qty(211.8, "cm") = #qty("2.118", "m")$, $V = #qty(4.258, "V")$ so
        $E = V / r = #qty(4.258, "V") / #qty("2.118", "m") = #qty("2.010", "V/m")$.
  ]

7. How does the electric field of 1 positive and 1 negative charge compare with the field from a single positive charge?

  Since $arrow(E) = - arrow(nabla) V = - ang(pdv(V, x), pdv(V, y))$, the magnitude
  $abs(E) = sqrt((pdv(V, x))^2 + (pdv(V, y))^2)$.

  We can use the approximations $pdv(V, x) approx (V_2 - V_1) / (Delta x)$ and
  $pdv(V, y) approx (V_2 - V_1) / (Delta y)$ to get
  $
    abs(E) approx sqrt(((V_2 - V_1) / (Delta x))^2 + ((V_2 - V_1) / (Delta y))^2).
  $

  + The electric field perfectly in the center of the two charges is $E = (2k q)/r^2$ where $r$ is the distance from the
    center to each charge and $q = #qty(1, "nC")$. Using the ruler tool, $r = #qty(75, "cm") = #qty("0.75", "m")$, which
    makes $E approx #qty(32, "N/C")$.

  + For two points with distance components $(Delta x, Delta y) = (#qty(0.2, "m"), #qty(0.2, "m"))$, we have
    $V_1 = #qty(-3.981, "V")$ and $V_2 = #qty(-2.867, "V")$. Plugging these values in, we get
    $E approx #qty(7.88, "N/C")$.

  + At another location with the same $Delta x$ and $Delta y$, I measured $V_1 = #qty(13.63, "V")$ and
    $V_2 = #qty(19.79, "V")$, which gives us $E approx #qty(41.1, "N/C")$.

  The electric field shoots out from the positive charge and curves towards the negative charge in all directions.

8. How does the electric field of a vertical line of positive charges and a negative charge at some distance from the
  line charge compare with the field from a single positive charge?

  The electric field shooting out from the line of positive charges is dominant, and as I increase the length of the
  line charge, the electric field becomes more uniform, stronger, and straight. There is a slight curve in towards the
  negative charge, especially when we are near it.
...
#table(
  ...
  table.header([*Coordinate*], [*Equivalent Position #_footnote1* (cm)], [*Electric Potential*, $V$ (volts)]),
  ..data
    .map(((coord, x, y, e-potential)) => {
      (
        coord,
...
Since the electric field is approximately uniform between the two electrodes and points in the same direction as the
horizontal path $Delta arrow(r)$, the formula for potential difference
$display(abs(Delta V) = abs(integral arrow(E) dot dd(arrow(r))))$ degrades to
$display(Delta V = arrow(E) dot Delta arrow(r) = E Delta x " " ==> " " cgreen(E = (Delta V) / (Delta x)))$.

Using $Delta V = 6 "V"$ and $Delta x = 6.7 "cm" = 0.067 "m"$, we get
$display(E = (Delta V) / (Delta x) = #qty(6, "V") / #qty("0.067", "m") = #cgreen($89.55 "V/m"$))$.

=== Experimental Calculation

Start with $Delta V = E Delta x$ from above. Since the electric potential is zero at $x = 0 "cm"$ from the left
electrode, we can simply write $V = E x$. From the line of best fit, I see that:
$
...
#box(inset: -12pt)[
  ...
  #grid(
    ...
    box(inset: 1em, stroke: 1pt, width: 1fr, align(center)[
      ...
      From $arrow(E) = - grad V$, I used the approximation $E approx -Delta V slash Delta s$ to estimate the electric
      field at various points in the grid.
...
+ What general statements can be made about the strength and characteristics of electric fields for the electrode plates
  that you mapped in the lab?

  The electric field between the two plates is approximately uniform since the equipotential lines seemed to be evenly
  spaced. This behavior was most apparent in the middle of the plates, but field lines began to slightly curve outward
...
+ Why is it not possible for two different equipotential lines to cross?
  ...
  If two equipotential lines crossed at some point $P$, there would be two different directions for the electric field
  at $P$, which would not make sense. A test charge placed at $P$ either moves in one direction or the other, but not
  both.

+ Under what conditions will the electric field between the electrodes of a parallel plate configuration be uniform?

  In the lab, I found that the electric field was the "most" uniform towards the center of the plates, where differing
  components of electric field could cancel out with each other. Thus, a parallel plate configuration where the plates
  are much larger than the distance (where it would be reasonable to call the size of the plates "infinite") between
  them will result in a uniform electric field between the plates. Each plate should also have a uniform charge
  distribution.

+ Why are the equipotential lines near a conducting surface parallel to the surface? \
  #subtext[Hint: since charges are free to move on a conductor, the electric field near a conductor is perpendicular to
    the conductor's surface]

  The electric field near the conductor's surface is perpendicular to the surface, and equipotential lines are always
  perpendicular to electric field lines. Equipotential lines are perpendicular to lines that are perpendicular, i.e.
...
+ Describe how a uniform electric field might be used to measure distance. \
  ...
  In a uniform electric field, the potential difference $Delta V$ between two points is directly proportional to the
  distance $Delta x$ between those points (we found $Delta V = E Delta x$). Thus, if we know the magnitude of the
  uniform electric field $E$, we can measure the distance between two points by measuring the potential difference
