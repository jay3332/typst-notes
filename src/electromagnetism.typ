...
Realize that this formula no longer depends on any inherent property of the capacitor, _just_ electric field. This means that it holds that _all_ types of capacitors, not just parallel plate, have the same formula for energy density. 
...
A series connection between $C_1$ and $C_2$ looks like the following: 
$ 
  #capacitor-series 
$
...
- Each charge carrier (e.g. electron) has a certain *drift velocity* $arrow(v)_d$ which is the average velocity of the charge carriers in the direction of current flow. The current density can also be expressed as: $
  ...
  #subtext[In other words, $rho = n q$, where $rho$ is volumetric charge density]

  - Drift velocities are often very small (on the order of $10^(-4) "m/s"$), but the electric field that causes the drift propagates at a significant fraction of the speed of light, so the effect of turning on a current is almost instantaneous.
...
#define("Electromotive Force")[
  The *electromotive force* (emf) $cal(E)$ of a voltage source is the amount of work done by the source to move a unit charge from the negative terminal to the positive terminal: $
    cal(E) = W / q.
  $
]
...
- The emf of a voltage source is the potential difference across the two terminals when _no current is flowing_.
...
- Batteries in real life have an *internal resistance* $r$. This means that the battery itself resists the flow of current, and thus the potential difference across the terminals is not equal to the emf $cal(E)$ of the battery. The true potential difference across the terminals is: $
  Delta V = cal(E) - I r.
$
  - We can technically represent internal resistance by coupling together an ideal battery and a resistor with resistance $r$.
...
- A *resistor* $#scale(resistor-symbol, 50%, reflow: true)$ is a circuit element that has a specific resistance $R$ and is used to control the flow of electric current in a circuit.
...
- Only _some_ resistors obey Ohm's Law, meaning that current through the resistor increases linearly with supplied voltage (i.e. $I prop Delta V$, and we call the constant of proportionality the resistance $R$). In other words, the resistance is constant, no matter the voltage supplied.
  ...
  - To check if a resistor is ohmic, check if $dv(V, R) = "some constant" I$.
...
In this case, it is important to keep track of the signs of $Delta V_i$.
