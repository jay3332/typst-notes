...
- Recall that for single-variable functions: $display(lim_(x->a) f(x) = L)$ means that by approaching $x=a$ from both the left and right, $f(x)$ approaches the same value $L$. When $display(lim_(x->a) f(x) = f(a))$, the function is _continuous_.

- For a function of two variables $f(x, y)$, we can approach some point $(x,y)=(a,b)$ from an infinite number of paths. If $f$ approaches the same limit $L$, then $display(lim_((x, y) -> (a,b)) f(x, y) = L)$.
...
- Let there be three functions of two variables $alpha, f, beta$.
  ...
  If, for a finite region $D$, $alpha(x, y) <= f(x, y) <= beta(x, y)$, then for any point $(a, b)$ in $D$ where $display(
    lim_((x, y) -> (a, b)) alpha(x, y) = lim_((x, y) -> (a, b)) beta(x, y) = L
)$, the *squeeze theorem* says that $display(lim_((x, y) -> (a, b)) f(x, y)) = L.$
...
#boxed(stroke: gray)[
  ...
  Let $g(rho, theta, phi)$ be the spherical transformation of $f(x, y, z)$, i.e. \ $g(rho, theta, phi) = f(rho sin phi cos theta, rho sin phi sin theta, rho cos phi)$. Then: $
...
#example("Limit using Polar Coordinates")[
  ...
  The limit depends on $theta$, which is the angle of the path we approach the origin from. In other words,
  the limit is different depending on the path, meaning our limit #tgreen[does not exist].
]
...
In other words, for any desired vertical distance $epsilon$, we must be able to find an $x$ a horizontal distance $delta$ away from $x=a$ where evaluating $f(x)$ is at most $epsilon$ away from $y=L$.

We can extend this to 3D: if $epsilon$ is the desired "error", then there must exist a point $(x, y)$ a radial distance $delta$ away from $(a, b)$ where evaluating $f(x, y)$ is at most $epsilon$ away from $L$.
...
- The *partial derivative* $pdv(f, x) = f_x (x, y, ...)$ of a function of several variables $f(x, y, ...)$ is the rate of change of $f$ as only one of the variables (in this case $x$) changes, and the other variables are held constant.

  - If $f$ is a function of $x$ and $y$, the partial derivative w.r.t. $x$ of $f$ at a point $(x, y) = (a, b)$, $evaluated(pdv(f, x))_((a, b)) =  f_x (a, b)$ is the rate of change of $f$ as $x$ varies while other $y$ is held constant near $(a, b)$.
...
#define("Limit Definition of a Partial Derivative")[
  ...
  ===== General Case
  Let $f(x_1, x_2, ..., x_n)$ be a function of $n$ variables. Then the *partial derivative* of $f$ with respect to $x_i$ is: $
...
#example([Find the Partial Derivatives of $f(x, y, z) = x e^y + y^2 e^z + z e^(-x).$])[
  ...
  $
    pdv(f, cblue(x)) &= pdv(,cblue(x)) (cblue(x) e^y + cgray(y^2 e^z) + z e^(-cblue(x)))
    &= e^y + -z e^(-x).
  $
  $
    pdv(f, cgreen(y)) &= pdv(,cgreen(y)) (x e^cgreen(y) + cgreen(y)^2 e^z + cgray(z e^(-x)))
    &= x e^y + 2y e^z.
  $
  $
    pdv(f, cpurple(z)) &= pdv(,cpurple(z)) (cgray(x e^y) + y^2 e^cpurple(z) + cpurple(z) e^(-x))
    &= y^2 e^z + e^(-x).
  $
]
...
A *second-order partial derivative* of a function of two variables $f(x, y)$ is the partial derivative of one of the partial derivatives of $f$. This means that there are *four* total second-order partial derivatives of f: $
  &  f_(x x) = npdv(f, x, 2)  #h(100pt) 
  && f_(x y) = (partial^2 f)/(partial y partial x) \
  
  &  f_(y x) = (partial^2 f)/(partial x partial y) #h(100pt) 
  && f_(y y) = npdv(f, y, 2) \
$
...
#define("Clairaut's Theorem")[
  Suppose a function $f(x, y)$ is _defined_ over a finite region $D$. If $f_(x y)$ and $f_(y x)$ are _continuous_ throughout $D$, then for all points $(x, y) in D$: $
...
- In a similar vein, a *third-order partial derivative* of a function is a partial derivative of one of the second-order partial derivatives of $f$. Generally, a function of $n$ variables has $n^3$ third-order partial derivatives. \
  #subtext[e.g. $f(x, y)$ has $8$ third-order partial derivatives: $f_(x x x), f_(x x y), f_(x y x), f_(x y y), f_(y x x), f_(y x y), f_( y y x), f_( y y y)$.]
...
#define("Differentiability by Partial Derivatives")[
  A function $f(x, y)$ is *differentiable* at a point $(a, b)$ if $f$ is both *continuous* on some closed finite region $D$ which includes $(a, b)$ and the partial derivatives $pdv(f, x)$ and $pdv(f, y)$ are both *defined* on the open region $D$. \
  #rsubtext[This can be extended to more than 2 variables.]
]

=== Partial Differential Equations

- An equation consisting of partial derivatives is called a *partial differential equation* (PDE).
  For example, $pdv(u, t) = alpha^2 npdv(u, x, 2)$ is a partial differential equation called the _wave equation_.
...
Recall that the derivative of a single-variable function can be interpreted as the _slope of the tangent line_ to the curve. A function of two variables defines a surface, there are an infinite number of tangent lines to the surface at any given point, however there exists a _plane_ that consists of all tangent lines. This plane is called the *tangent plane*.
...
We can use this tangent plane to derive a general function called the _linearization function_ which can be used to approximate values of $f$ near the point: #define("Linearization of a Function of Two Variables")[
...
Recall the tangent plane equation. If the point of interest $(x_0, y_0, z_0)$ is made infinitesimally close to an arbitrary point $(x, y, z)$, then the difference of each component is also infinitesimally small. That is, if $x - x_0 -> 0$, we can call this difference $dd(x)$, called the *differential* of $x$.
...
#example([Interpreting differentials as finite errors])[
  ...
  The cone is measured this way to have a height of $14 "mm"$ and a radius of $6 "mm"$, resulting in an estimated volume of $168 pi space.quarter "mm"^3$. What is the approximate max error of the estimate?

  #lorange

  Using the total differential from the other example, we can estimate the error $dd(V)$ from the measurement. Since $dd(r)$ can be interpreted as the "error in $r$", we can set $dd(r) = 4 "mm"$. Similarly, $dd(h) = 3 "mm"$. Finally, $r = 6 "mm"$ and $h = 14 "mm"$.
...
#note(title: "Differentials vs. Increments")[
  The *increment* of a function $f$, represented $Delta f$ is the _true_ change in $f$ over some change in its variables $arrow(r)$, represented $Delta arrow(r)$. A *differential* uses a tangent line to approximate the increment ($dd(f) approx Delta f$), and the approximation gets better as $dd(arrow(r)) -> arrow(0)$.
]

=== Tangent Planes to Parametric Surfaces

- Recall that a parametric surface is defined by a vector function of two variables, for example $arrow(r)(u, v) = ang(x(u, v), y(u, v), z(u, v))$.

- The tangent plane to a parametric surface at a point $P$ can be found using the cross product of the two tangent vectors $pdv(arrow(r), u)$ and $pdv(arrow(r), v)$.

  - Originating from $P$, all tangent vectors can be thought of as "lying on the tangent plane" 

  - Thus normal vector $arrow(n)$ to the tangent plane is orthgonal to any two tangent vectors at $P$.
    We can choose the two most convenient ones, $pdv(arrow(r), u)$ and $pdv(arrow(r), v)$: $
    arrow(n) = pdv(arrow(r), u) times pdv(arrow(r), v).
  $

  - If $arrow(n) = ang(a, b, c)$, then the equation of the tangent plane at point $P(x_0, y_0, z_0)$ is $a(x - x_0) + b(y - y_0) + c(z - z_0) = 0$ by the standard normal-point form of a plane.

#example("Tangent Plane to a Parametric Surface")[
  Find the equation of the plane tangent to the surface defined by $arrow(r)(u, v) = ang(u cos v, u sin v, v)$ at the point $(0, 1, pi/2)$.
...
- The last three terms are the parts of the approximation involving the "second-derivative". There are actually four terms, but since $f_(x y) = f_(y x)$, the two mixed partial derivative terms can be combined (hence why there is no division by $2$)
...
- For single-variable functions $y = f(x)$, $dv(y, x) = dv(y, u) dv(u, x)$ if there is an "intermediate function" $u(x)$ where $y$ is a function of $u$.
...
#grid(columns: 2, [
  - We can use a *tree diagram* to help visualize the chain rule. Each node is a variable, and each edge represents
...
- To differentiate an implicitly defined function, we use the chain rule just like with single-variable functions.
  ...
  - If $z(x, y)$ is implicitly defined, then there are two derivatives we can find: $pdv(z, x)$ or $pdv(z, y)$.
    - To find $pdv(z, x)$, take $pdv(,x)$ of both sides, use the chain rule when you see a $z$, then solve for $pdv(z, x)$.
    - To find $pdv(z, y)$, take $pdv(,y)$ of both sides, use the chain rule when you see a $z$, then solve for $pdv(z, y)$.
...
#example("Implicit Differentiation")[
  ...
  $ 
  Let $u = z^3$. To find $pdv(,x) (z^3) = pdv(u, x)$, we must use the chain rule, since $u$ is not an independent variable. Since $u$ is a function of $z$ is a function of $x$, the chain rule tells us that: $
    pdv(u, x) = pdv(u, z) pdv(z, x).
  $

  Since $pdv(u, z) = 3z^2$, $pdv(u, x) = pdv(,x) (z^3) = 3z^2 pdv(z, x)$. Similarly we can find using the product rule that $pdv(, x)(6 x y z) = 6y pdv(,x) (x z) = 6y z + 6 x y pdv(z, x)$: $
...
If we have a function $z(x, y)$ implicitly defined by the equation $F(x, y, z) = 0$, we can find both $pdv(z, x)$ and $pdv(z, y)$ by taking the partial derivative of both sides w.r.t. $x$ and $y$, respectively: $
...
The *gradient* is a vector of all partial derivatives of a function. If $f$ is a function that takes in $n$ parameters, then the gradient of $f$, typically denoted $grad f$, is an $n$-dimensional vector.
...
Take the total differential of a function $f(x, y)$, $dd(f) = pdv(f, x) dd(x) + pdv(f, y) dd(y)$. We can represent this as the following dot product: $
  dd(f) = ang(pdv(f, x), pdv(f, y)) dot ang(dd(x), dd(y)).
$
If we let $dd(arrow(r)) = ang(dd(x), dd(y))$ (a change in "position"), then the total differential can be written as just: $
  dd(f) = grad f dot dd(arrow(r)).
$

=== Directional Derivatives

- Suppose a surface $z = f(x, y)$ is graphed in the 3D-plane. Each partial derivative is the slope of the tangent line in the direction of the respective axis. For example, $pdv(z, x)$ is the slope of the tangent line in the direction of $+x$. 
...
- The directional derivative $D_arrow(v) space f(x, y)$ is the slope of the line tangent to the surface _in the direction_ of $arrow(v)$ is denoted $D_arrow(v) f(x, y)$.

#define("Directional Derivative")[
  Let $z = f(x, y)$ be a surface in the 3D-plane, and $hat(v) = ang(v_1, v_2)$ is a 2D #underline[unit] vector. The *directional derivative* in the direction of $hat(v)$ is: $
...
- An $n$-dimensional manifold has a directional derivative in the direction of an $(n-1)$-dimensional vector (e.g. a 4D shape has a 3D directional vector)
...
- In the same way, the direction of *steepest descent* is in the direction of $-grad f(x, y)$, and the minimum value of the directional derivative is $-abs(grad f(x, y))$.

- If the angle between the direction vector and the gradient vector is $theta$, then by the definition of the dot product, $
...
Using the gradient vector, we can generalize the linear approximation for a multivariate function $f$, given its "center" $arrow(r)_0 = ang(x_0, y_0, z_0, ...)$ (i.e. the "given point"): $
...
#define("Hessian Operator (Gradient Definition)")[
  The *hessian operator* $bf(H)$, when applied to a function, returns a matrix of its second-order partial derivatives. The operator is defined as: $
...
#define("Hessian Matrix")[
  Let $f(x_1, x_2, ..., x_n)$ be a function of $n$ variables. Then the *Hessian matrix* of $f$ is the matrix of all partial derivatives: $
    bf(H)(f) = mat(
  ...
  $
  Or, more compactly, it is the $n times n$ matrix $bf(H)$ where $bf(H)_(i, j) = (partial^2 f)/(partial x_i partial x_j)$. 
]
...
#define("Critical Points")[
  Let $f$ be a function of several variables: $f(x, y, ...)$. $f$ has a *critical point* at $(a, b, ...)$ if $grad f(a, b, ...) = arrow(0)$ or it is not differentiable at $(a, b, ...)$ (i.e. $grad f$ is undefined). 
]

A local extrema does not exist at every critical point. To determine if a critical point is a local extrema, we can use the _second partial derivative test_:

#define("Second Partial Derivative Test")[
  Let $f(x, y)$ be a function of two variables whose second-order partial derivatives are *continuous*. Let $D = det(bf(H)(f))$ be the determinant of the _Hessian matrix_ of $f$. \ In other words:
...
- A function $f(x, y)$ as an _absolute maximum_ at $(a, b)$ on a  region $D$ if $f(a, b) >= f(x, y)$ for all points $(x, y)$ in $D$.
- A function $f(x, y)$ as an _absolute minimum_ at $(a, b)$ on a region $D$ if $f(a, b) <= f(x, y)$ for all points $(x, y)$ in $D$.
- Collectively, these are called _absolute extrema_.

#define("Extreme Value Theorem")[
  Let $f(x, y)$ be a function of two variables that is *continuous* on a _closed_ and _bounded_ region $D$. Then $f$ attains both an absolute maximum and an absolute minimum on $D$. 
]
...
#subtext[Essentially, the absolute extrema can occur either in the interior of the region (which we can easily find candidates for using $grad f = arrow(0)$) or on the boundary of the region. Unlike single-variable functions, there are an infinite number of points at the boundary.
...
- Find the absolute extrema of $f$ on parts of the boundary where we can reduce $f$ to a single-variable function (e.g. along line segments) --- this is called the _closed interval method_.
...
#example("Closed Interval Method with Line Segments")[
  Find the absolute extrema of $f(x, y) = x^2 - 2x y + 2y$ on the triangular region with vertices at $(0, 0), (2, 0), (0, 2)$.
  #lorange
  ...
  - $L_1$ is the line segment from $(0, 0)$ to $(2, 0)$. Here, $y = 0$, so $f(x, 0) = x^2$.
    ...
    - $f(x, 0)$ has a critical point at $x=0$ since it satisfies $dv(,x) f(x,0) = 2x = 0$. We must also consider endpoints at $x=0$ and $x=2$. Add $(0, 0), (2, 0)$ as candidates.

  - $L_2$ is the line segment from $(2, 0)$ to $(0, 2)$. Here, $y = -x + 2$, so $f(x, -x + 2) = x^2 - 2x(-x + 2) + 2(-x + 2) = 3x^2 - 6x + 4$.

    There is a critical point at $x=1$ ($dv(,x) f(x, -x + 2) = 6x - 6 = 0 ==> x=1$) and endpoints at $x=0$ and $x=2$. Add $(1, 1), (2, 0), (0, 2)$ as candidates.
...
- The _closed interval method_ works when the boundary can be reduced to single-variable functions (e.g. line segments), allowing us to determine a finite set of _candidates_ for which we can test the function at to find the absolute extrema. 
  - However, this method does not work for more complex boundaries (e.g. circles, ellipses, curves, etc.).

- The method of *Lagrange Multipliers* allows us to achieve this step. It is an alternative method of finding a finite set of _candidates_ on the boundary.
...
- At the points where $f$ attains its absolute extrema on the constraint, the level curve of $f$ is _tangent_ to the level curve of $g$.
  - This means that the two curves have the same slope at these points.

- The gradients of $f$ and $g$ are orthogonal to their respective level curves. Thus, at the points where $f$ attains its absolute extrema on the constraint, the gradients of $f$ and $g$ are _parallel_.
...
+ Solving this system gives us a finite number of _candidates_ $(x, y)$ for which we can evaluate $f(x, y)$ to find the absolute extrema subject to the constraint.
                                    
#define("Method of Lagrange Multipliers with One Constraint")[
  The _candidates_ for the absolute extrema of a function $f(x, y)$ subject to the constraint $g(x, y) = c$ occur for all $(x, y, lambda)$ that satisfy the system of equations: $
...
+ Suppose the boundary of $f$ is the constraint $g(x, y) = c$. Then, we can use the method of Lagrange multipliers to find candidates on the boundary.
...
+ Suppose we want to find the absolute extrema of a function $f(x, y, z)$ subject to two constraints: $g(x, y, z) = c_1$ and $h(x, y, z) = c_2$ (where $c_1$ and $c_2$ are constants).
  ...
  - The constraint can also be thought of as the _curve of intersection_ between the two surfaces defined by $g(x, y, z) = c_1$ and $h(x, y, z) = c_2$.

+ At the points where $f$ attains its absolute extrema on the constraints, the level surface of $f$ is tangent to both constraint surfaces.
...
+ The second and third constraints are the original constraints: $g(x, y, z) = c_1$ and $h(x, y, z) = c_2$.
  Together, we have a system of five scalar equations with five unknowns $(x, y, z, lambda, mu)$: $
...
#define("Method of Lagrange Multipliers with Multiple Constraints")[
  The _candidates_ for the absolute extrema of a function $f(x, y, z)$ subject to the constraints $g(x, y, z) = c_1$ and $h(x, y, z) = c_2$ occur for all $(x, y, z, lambda, mu)$ that satisfy the system of equations: $
...
#example("Absolute Extrema of a Function of Three Variables")[
  Let $f(x, y, z) = x^2 + 2y^2 - 3 x y + z^2$. The closed curve $C$ is defined by the intersection of the surfaces defined by $x^2 + y^2 + z^2 = 6$ and $x + y + z = 0$. The region $D$ is the set of all points on and inside the curve $C$. Find the absolute extrema of $f$ on the region $D$.
  #lorange
  First, find the critical points in the interior of the region by setting $grad f = arrow(0)$. $
  ...
  $
  Thus, the only critical point in the interior is $(0, 0, 0)$.
  Next, use the method of Lagrange multipliers to find candidates on the boundary defined by the constraints $g(x, y, z) = x^2 + y^2 + z^2 = 6$ and $h(x, y, z) = x + y + z = 0$.
